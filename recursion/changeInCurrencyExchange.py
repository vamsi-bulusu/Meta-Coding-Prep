...
def canGetExactChange(targetMoney, denominations):
  # can reuse denominations to form targetmoney
  def isPossible(index, currentSum):
    
    if currentSum == targetMoney:
      return True
    
    if index == len(denominations) or currentSum > targetMoney:
      return False
    
    ans = False
    for i in range(index, len(denominations)):
      ans = ans or isPossible(i, currentSum + denominations[i])
