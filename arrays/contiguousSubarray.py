...
def count_subarrays(arr):
  ...
  def find_next_greater(arr, ans):
    ...
    while(len(stack) > 0):
      ...
      ans[arr[stack[-1]]] += len(arr) - (stack[-1] + 1)
      stack.pop()
      
    return ans
  
  countMap = defaultdict(int)
  countMap = find_next_greater(arr, countMap)
  countMap = find_next_greater(list(reversed(arr)), countMap)
  res = [1]*len(arr)
  for index, num in enumerate(arr):
    res[index] += countMap[num]
