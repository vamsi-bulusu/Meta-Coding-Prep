...
the ith of which is the 1-based index in the original array of the element 
...
the largest one is the 4, which was at index 5 in the original array. 
...
def findPositions(arr, x):
  queue = [(i + 1, val) for i, val in enumerate(arr)]
  res = []
  for i in range(1, x + 1):
    tmp = []
    mi, mx, l = -1, -1, len(queue)
    for _ in range(min(x, l)):
      ci, cx = queue.pop(0)
      if cx > mx:
        mx = cx
        mi = ci
      tmp.append((ci, max(0, cx - 1)))
